...
namespace JWTAuthentication.WebApi.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task<ApplicationUser> UpdateAsync(UpdateModel model)
        {
        ...
        public async Task<AuthenticationModel> GetTokenAsync(TokenRequestModel model)
        {
            ...
            //{
            //    authenticationModel.IsAuthenticated = false;
            //    authenticationModel.Message = $"{user.Email} is blocked.";
            //    return authenticationModel;
            //}
        ...
        public async Task<List<ApplicationUserDTO>> GetUsersAsync(int type)
        {
            var users = new List<ApplicationUser>();
            List<ApplicationUserDTO> data = new List<ApplicationUserDTO>();
            ...
            foreach (ApplicationUser user in _userManager.Users)
            {
                if (type == 1)
                {
                    ...
                    {
                        ApplicationUserDTO single = new ApplicationUserDTO();
                        var rolename = await _userManager.GetRolesAsync(user);

                        single.Role = rolename.ToArray()[0];
                        single.ApplicationUsers = user;
                        data.Add(single);
                    }
                }
                else if(type == 2)
        ...
        public async Task<IEnumerable<ApplicationUser>> GetUsersByRole(string rname)
        {

            var users = new List<ApplicationUser>();
            foreach (ApplicationUser user in _userManager.Users)
            {
                //var rolename = _roleManager.FindByIdAsync(rname).Result.Name;
                if (await _userManager.IsInRoleAsync(user, rname))
                    users.Add(user);
            }
        ...
        public async Task<bool> BlockUnblockUserAsync(string uid)
        {
            ApplicationUser user = await _userManager.FindByIdAsync(uid);
